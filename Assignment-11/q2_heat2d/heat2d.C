...
main(int argc, char *argv[])
{
  ...
  const string matlabFileNameGPU = "GPU_" + matlabFileName;
  heat2dSolveGPU(debug, nx, ny, numSteps, numGhost, n1a, 
      n1b, nd1, nd1a, nd1b, n2a, n2b, nd2, nd2a, nd2b, saveMatlab, 
      tFinal, rx, ry, dt, cpuTimeStep, maxErr, gridIndexRange, 
      dimension, boundaryCondition, Rx, Ry, x, dx, ua, matlabFileNameGPU);
  if( saveMatlab )
  {
    FILE *matlabFile = fopen(matlabFileNameGPU.c_str(),"w");
    fprintf(matlabFile,"%% File written by heat2d.C\n");
    fprintf(matlabFile,"xa=%g; xb=%g; ya=%g; yb=%g; kappa=%g; t=%g; maxErr=%10.3e; cpuTimeStep=%10.3e;\n",
    xa,xb,ya,yb,kappa,tFinal,maxErr,cpuTimeStep);
