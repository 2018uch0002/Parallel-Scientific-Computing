      ...
      int* nd1a_d, int* n2a_d, int* n2b_d, int* nd2_d, int* nd2a_d)
{
  ...
  if ( (i1 >= n1a && i1 <=n1b) && (i2 >= n2a && i2 <= n2b) )
  {
    ...
    // if((i1 == 5) && (i2==5))
    //   printf("t=%12.4e, UN(%d,%d)= %12.4e\tU(i1,i2)= %12.4e\tU(i1-1,i2)=%12.4e"
    //          "U(i1+1,i2)= %12.4e\tU(i1,i2-1)=%12.4e\tU(i1,i2+1)=%12.4e\n",
    //         t, i1, i2, UN(i1,i2), U(i1,i2), U(i1-1,i2), U(i1+1,i2),
    //         U(i1,i2-1), U(i1,i2+1));
  }
}
        ...
        int i2, int is)
{
  ...
  if ((nd1a <= i1) && (i1 <= nd1b))
  {
    ...
    // if (i1==20 && i2 == 20)
      // printf("t=%12.4e, UN(%d,%d)= %12.4e, \tUN(i1, i2g)= %12.4e\t"
      //        "UN(i1,i2+is)= %12.4e\tUN(i1,i2+2*is)=%12.4e\n",
      //       t, i1, i2, UN(i1, i2), UN(i1, i2g), UN(i1,i2+is), UN(i1,i2+2*is));
  }
}
  ...
  RealArray& x, const Real *dx, RealArray *ua, const std::string& matlabFileNameGPU)
{
  ...
  for( n=0; n<numSteps; n++ )
  {
      ...
      for( int side=0; side<=1; side++ )
      {
        ...
        if( boundaryCondition(side,axis)==dirichlet )
        {
          ...
          { // left or right side
            ...
            //   // if (i2 == 6)
            //   // printf("1: INSIDE: %d, %d, %d, %d, %d, %12.4e, %12.4e \n", 
            //   //     axis, side, i1, i1g, i2, un(i1, i2), un(i1g, i2));
            // }
            // if (i1 == 4)
            // printf("INSIDE: %d, %d, %d, %d, %d, %12.4e, %12.4e \n", 
                  // axis, side, i1, i1g, i2, un(i1, i2), un(i1g, i2));
          }
          else
          { // bottom or top
            ...
            //   un(i1,i2) = UTRUE(x(i1,i2,0),x(i1,i2,1),t+dt);
            //   un(i1,i2g) = 3.*un(i1,i2) - 3.*un(i1,i2+is) + un(i1,i2+2*is); // extrap ghost
            //   //  printf("2: INSIDE: %d, %d, %d, %d, %d, %12.4e, %12.4e \n", 
            //   //     axis, side, i1, i2, i2g, un(i1, i2), un(i1, i2g));
            // }
            // // if (i2 == 6)
            // printf("INSIDE: %d, %d, %d, %d, %d, %12.4e, %12.4e \n", 
            //       axis, side, i1, i2, i2g, un(i1, i2), un(i1, i2g));
    ...
    // printf("GPU [OUTSIDE] :  u(%d, %d) = %12.4e \n", 4, 5+1, un(4, 5+1));
    // cudaDeviceSynchronize();
