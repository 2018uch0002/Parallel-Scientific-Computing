...
constexpr int numberOfDimensions = 1;
...
void heat2dSolveGPU(const int& debug, const int& nx, const int& ny, const int& numSteps,
  ...
  const double& ry, const double& dt, IntegerArray& gridIndexRange, 
  IntegerArray& dimension, IntegerArray& boundaryCondition, Range& Rx, Range& Ry, 
  RealArray& x, const Real *dx, RealArray *ua, const std::string& matlabFileNameGPU);
}
      ...
      int* nd1a_d, int* n2a_d, int* n2b_d, int* nd2_d, int* nd2a_d)
{
  ...
  if ( (i1 >= n1a && i1 <=n1b) && (i2 >= n2a && i2 <= n2b) )
  {
    ...
    if ((i1 ==4 && i2==5) || (i1 ==2 && i2==4))
    printf(">>>: t=%12.4e, rx=%12.4e,\t U(%d. %d)=%12.4e,\t U+1 %12.4e,\t U-1 %12.4e \n",
            t, rx, i1, i2, U(i1, i2), U(i1, i2+1), U(i1, i2-1));
    // printf(">>>: t=%12.4e\t, UN(%d, %d) = %12.4e \t %12.4e \tuc %12.4e \trx=%12.4e \try=%12.4e \t last=%12.4e \t ans= %12.4e\n ", 
    //       t, i1, i2, 
    //       UN(i1, i2), UTRUE( X(i1,i2,0), X(i1,i2,1), t),
    //       U(i1,i2), 
    //       rx*( U(i1+1,i2) -2.*U(i1,i2) + U(i1-1,i2) ), 
    //       ry*( U(i1,i2+1) -2.*U(i1,i2) + U(i1,i2-1) ), 
    //       dt * FORCE( X(i1,i2,0), X(i1,i2,1), t ),
    //       U(i1,i2) + rx*( U(i1+1,i2) -2.*U(i1,i2) + U(i1-1,i2) )
    //           + ry*( U(i1,i2+1) -2.*U(i1,i2) + U(i1,i2-1)) 
    //           + dt * FORCE( X(i1,i2,0), X(i1,i2,1), t )  );
  }
  ...
  RealArray& x, const Real *dx, RealArray *ua, const std::string& matlabFileNameGPU)
{
  ...
  //     printf(">>>U(%d, %d) = %12.4e \t %12.4e %s\n", 
  //       i1, i2, u0(i1, i2), 
  //       UTRUE(x(i1,i2,0),x(i1,i2,1),t),
  //       u0(i1, i2)==UTRUE(x(i1,i2,0),x(i1,i2,1),t) ? "TRUE" : "FALSE");
      
  //       // printf(">>> X(%d, %d) = %12.4e \t %12.4e \t %s \n", i1, i2, 
  //     //   x(i1, i2, axis), 
  //     //   x_p[ (i1-nd1a) + nd1 * (i2-nd2a) + axis * nd1 * nd2] ,
  //     //   x(i1, i2, axis) == x_p[ (i1-nd1a) + nd1 * (i2-nd2a) + axis * nd1 * nd2] ? "True" : "FALSE"
  //     //   );
  //   }
  ...
  for( n=0; n<2; n++ )
  {
    ...
    double* un_p = ua[next].getDataPointer();
    cudaMemcpy(un_p, u_d[next], size_of_array, cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(u_d[next], un_p, size_of_array, cudaMemcpyHostToDevice);
    cudaDeviceSynchronize();
  ...
  printf("I1 %d, I2 %d uc=%12.4e, TRUE=%12.4e maxERR=%12.4e \t %12.4e\n", 
    max_I1, max_I2, 
    uc(max_I1, max_I2), 
    UTRUE(x(max_I1, max_I2, 0), x(max_I1, max_I2, 1), tFinal),
    fabs(uc(max_I1, max_I2) - UTRUE(x(max_I1, max_I2, 0), x(max_I1, max_I2, 1), tFinal)),
    maxErr / max(maxNorm,REAL_MIN) );

  maxErr /= max(maxNorm,REAL_MIN); // relative error

  printf("GPU: numSteps=%d nx=%d maxNorm=%8.2e maxRelErr=%8.2e cpuTimeStep=%9.2e(s)\n\n\n",
    numSteps,nx,maxNorm,maxErr, 0.0);
