...
int main( int argc, char* argv[] )
{
    ...
    Real *u_p[2] = {NULL, NULL}; // two arrays will be used for current and new times
    u_p[0] = new Real [nd1];
    u_p[1] = new Real [nd1];
    // if (myRank == 0){
    //     u_p[0] = new Real [nd1];
    ...
    for( int n=0; n<numSteps; n++ )
    {
        ...
        for( int side=0; side<=1; side++ )
        {
            ...
            printf(">> myrank = %d, side = %d \n", myRank, side);
            if( boundaryCondition(side,0)==dirichlet )
            {
                un(i) = UTRUE(x(i),t+dt);
                un(i-is) = 3.*un(i) - 3.*un(i+is) + un(i+2*is); // extrapolate ghost

                // since rank-0 will come here if side==0 when all the interio ranks will have parallel ghost bc
                // so send the right-side value
                if (side == 0)
                {
                    if (myRank == 0)
                    {
                        int send_tag = 100 * myRank + 1; // send tag for the right side point 
                        // send the right side value at n1b_l
                ...
                else if (side == 1)
                {
                    if (myRank == (np-1))
                    {
                        int send_tag = 100 * myRank + 0; // send tag for the left side point 
                        // send the left side value at n1a_l to the previous rank
                        MPI_Send(&uc(n1a_l), 1, MPI_DOUBLE, myRank-1, send_tag, MPI_COMM_WORLD);
                    }
                }
            }
            else if (boundaryCondition(side,0)==parallel_ghost)
            {
                if (side == 0) // left side boundary conditions
                {
                    // first ask all the even ranks to send the values to odd ranks
                    ...
                    {   // odd ranks are receiving value from previus rank for the left side evaluation.
                        ...
                        uc(i+1) = uc_nd1b_l;
                        un(i) = uc(i) + rx*( uc_nd1b_l - 2.*uc(i) + uc(i-1) ) + dt*FORCE( x(i),t );
                        printf("rec\n");
                    }
                    MPI_Barrier(MPI_COMM_WORLD);

                    // now, odd rank will send the left side value and previous even ranks will receieve it.
                    if (myRank%2 == 1)
                ...
                {
                    // first odd ranks will send the left side value for even rank's right side value evaluation
                    if (myRank%2 == 1)
                    {   // odd rank will send the right side value
                        int send_tag = 100 * myRank + 0; // send tag for the left side value
                        MPI_Send(&uc(n1a_l), 1, MPI_DOUBLE, 1, send_tag, MPI_COMM_WORLD);
                    }
                    else 
                    {
                        ...
                        uc(i-1) = uc_nd1a_l;
                        un(i) = uc(i) + rx*( uc(i+1) - 2.*uc(i) + uc_nd1a_l ) + dt*FORCE( x(i),t );
                    }
                    MPI_Barrier(MPI_COMM_WORLD);
                    ...
                    if (myRank%2 == 0)
                    {
                        // odd rank will send the right side value
                        int send_tag = 100 * myRank + 0; // send tag for the left side value
                        MPI_Send(&uc(i), 1, MPI_DOUBLE, myRank+1, send_tag, MPI_COMM_WORLD);
                    }
                    else
                    {
                        int recv_tag = 100 * (myRank+1) + 0;
                ...
                // // if rank is zero, we have to send right side end value to 
                // // next rank-1 and recieve left side from ranlk-1
                // if (myRank == 0){
                //     // send the value 
                //     int send_tag = 10; //myRank * 100 + is;
                //     // printf("%d, myRank %d, send %12.4e, i =%d \n", n, myRank, uc(i), i);
                //     MPI_Send(&uc(i), 1, MPI_DOUBLE, 1, send_tag, MPI_COMM_WORLD);

                //     // now the recive the values of rank-1
                //     Real uc_nd1b_l = 0.; 
                //     MPI_Status status;
                //     int recv_tag = 1; //(myRank+is) * 100 + is;
                //     MPI_Recv(&uc_nd1b_l, 1, MPI_DOUBLE, 1, recv_tag, MPI_COMM_WORLD, &status);
                //     // printf("%d, myRank %d, recv %12.4e, i =%d \n", n, myRank, uc_nd1b_l, i);
                ...
                //     Real uc_nd1a_l = 0.; 
                //     MPI_Status status;
                //     int recv_tag = 10; //(myRank+is) * 100 + is;
                //     MPI_Recv(&uc_nd1a_l, 1, MPI_DOUBLE, myRank-1, recv_tag, MPI_COMM_WORLD, &status);
                //     uc(i-1) = uc_nd1a_l;
                //     un(i) = uc(i) + rx*( uc(i+1) - 2.*uc(i) + uc_nd1a_l ) + dt*FORCE( x(i),t );
                //     // printf("%d, myRank %d, recv %12.4e, i =%d \n", n, myRank, un(i), i);
                    
                //     // send value to the left
                //     int send_tag = 1; //myRank * 100 + is;
                //     // printf("%d, myRank %d, send %12.4e, i =%d \n", n, myRank, uc(i), i);
                //     MPI_Send(&uc(i), 1, MPI_DOUBLE, myRank-1, send_tag, MPI_COMM_WORLD);
                // } else {
                //     if (myRank % 2 == 1)
    ...
    else if (myRank == np-1) itr_end = nd1b_l;

    for( i=itr_start; i<=itr_end; i++ )
    {
        error(i) = uc(i) - UTRUE(x(i),t);
        maxErr_local = max( maxErr_local, abs(error(i)) );
        // printf("myRank %d, uc(%d)= %12.4e, error(%d) = %12.4e  \n", myRank, i, uc(i), i, error(i) );
    }
