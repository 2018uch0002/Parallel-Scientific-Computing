...
main(int argc, char *argv[])
{
  ...
  Real cpu1 = getCPU();
  // if(myRank == 0) printf("numSteps = %d \n", numSteps);
  printf("myRank = %d, nd2a_l=%d, nd2b_l=%d \n", myRank, nd2a_l , nd2b_l);
  numSteps = 1;
  for( n=0; n<numSteps; n++ )
  {
    ...
    /*
    // for side = 0 ; send the data from here to next rank 
            // (this rank will be zero-rank)
            if (np>1 && (side == 0) && (myRank == 0))
            {
              int sending_i2_index = gridIndexRange(side,axis);
              // sendt-tag for the top part
              int send_tag = myRank * 100 + 1; // 0: bottom 1: top
              double *u_p = u.getDataPointer();
              MPI_Send(u_p + nd1*(sending_i2_index-nd2a_l), nd1, MPI_DOUBLE, 
                      myRank+1, send_tag, MPI_COMM_WORLD);
            }

            // for side = 1; send the data from here to previous rank 
            // (this rank will be (np-1)-rank)
            if (np>1 && (side == 1) && (myRank==np-1))
            {
              int sending_i2_index = gridIndexRange(side,axis);
              // sendt-tag for the top part
              int send_tag = myRank * 100 + 0; // 0: bottom 1: top
              double *u_p = u.getDataPointer();
              MPI_Send(u_p + nd1*(sending_i2_index-nd2a_l), nd1, MPI_DOUBLE, 
                      myRank-1, send_tag, MPI_COMM_WORLD);
            }
    ...
    if (np>1)
    {
      for( int side=0; side<=1; side++ ) // only for top and bottom
      {
        ...
        if ( can_recv )
        {
          ...
          // if(n == numSteps-1 && side==0){
          //   printf("RECV>> myRank=%d, recv_from_rank=%d, recv_tag=%d, i2=%d, i2g=%d, side=%d \n",
          //         myRank, recv_from_rank, recv_tag, i2, i2g, side);
          // }
          ...
          // {
          //   for(i1 = nd1a; i1<=nd1b; i1++)
          //     printf("Recv::  myRank = %d, side=%d, u(%d, %d) = %12.4e\t %d\n", myRank, side, i1, i2g, u(i1, i2g), i2g-nd2a_l);
          // }
        }
        ...
        if ( can_send )
        {
          ...
          // if(n == numSteps-1 && side==0){
          //   printf("SEND-- myRank=%d, send_to_rank=%d, send_tag=%d, i2=%d, i2g=%d, side=%d, opposide=%d\n",
          //         myRank, send_to_rank, send_tag, i2, i2g, side, opposite_side);
          // }

          // const int sending_i2_index = gridIndexRange(opposite_side, 1);
          ...
          // {
          //   for(i1 = nd1a; i1<=nd1b; i1++)
          //     printf("Send:: myRank = %d, side=%d u(%d, %d) = %12.4e \t%d\n", 
          //           myRank, side, i1, i2, u(i1, i2), i2-nd2a_l);
          // }
        }
        ...
        //   {
        //     for(i1 = nd1a; i1<=nd1b; i1++)
        //       printf("Recv::  myRank = %d, u(%d, %d) = %12.4e\n", myRank, i1, i2, u(i1, i2));
        //   }
        //   MPI_Barrier(MPI_COMM_WORLD);
        ...
        //   {
        //     for(i1 = nd1a; i1<=nd1b; i1++)
        //       printf("Send:: myRank = %d, u(%d, %d) = %12.4e\n", myRank, i1, i2, u(i1, i2));
        //   }
        //   MPI_Barrier(MPI_COMM_WORLD);
        // }
        // update the boundary
    ...
    for( i2=nd2a_l; i2<=nd2b_l; i2++ )
      {
        for( i1=nd1a; i1<=nd1b; i1++ )
        {
          printf("x(%d, %d, %d) = %12.4e\t", i1, i2, k, x(i1, i2, k));
        }
        printf("\n");
      }
  }
  if (np>1)
  {
    if (myRank!=0)
    {
      ...
      printf(">>> myRank=%d, buffsize %d  %d %d\n", myRank, buff_size, nd2_l, nd1);
      int count = 0;
      int base1 = nd1a;// x.getRawBase(0);
      int base2 = nd2a_l; //x.getRawBase(1);
      int base3 = 0; //x.getRawBase(2);

      int len1  = nd1; // x.getLength(0);
      int len2  = nd2_l; // x.getLength(1);
      int len3  = 2; //x.getLength(2);
      for (int k = 0; k<2; k++)
        for( i2=nd2a_l+1; i2<=nd2b_l; i2++ )
        {
          for( i1=nd1a; i1<=nd1b; i1++ )
          {
            printf("x(%d, %d, %d) = %12.4e  %p % 12.4e \n", 
                    i1, i2, k, x(i1, i2, k), (x_p + count), 
                    *(x_p + (i1 - base1)+ (i2 - base2) * len1 + (k  - base3) * (len1 * len2)) );
      ...
      printf("x(%d, %d, %d) = %12.4e % 12.4e \n", 
                    nd1b, nd2b_l, 0, x(nd1a, nd2b_l, 0), 
                    *(x_p + (nd1b - base1)+ (nd2b_l - base2) * len1 + (0  - base3) * (len1 * len2)) );

      printf("count= %d base1 %d, base2 %d base3 %d len1 %d len %d, len %d\n", 
              count, base1, base2, base3, len1, len2, len3);
      MPI_Send(&buff_size, 1, MPI_INT, 0, myRank, MPI_COMM_WORLD);
      
      // send the x
      
      MPI_Send(x_p + (nd1a - base1)+ (nd2a_l+1 - base2) * len1 + (0  - base3) * (len1 * len2),
                     buff_size, MPI_DOUBLE, 0, myRank * 10, MPI_COMM_WORLD );

      // MPI_Send(x_p + (nd1a - base1)+ (nd2a_l+1 - base2) * len1 + (1  - base3) * (len1 * len2),
      //               buff_size, MPI_DOUBLE, 0, myRank * 100, MPI_COMM_WORLD );
    ...
    else
    {
      MPI_Status status;
      int buff_size = 0;
      int offset = 2 * nd1 * (n2b_l - n2a_l+1 + numGhost);
      double* x_p = x.getDataPointer();
      for (int r = 1; r < np; r++)
      {
        
        MPI_Recv(&buff_size, 1, MPI_INT, r, r, MPI_COMM_WORLD, &status);
        printf("RECV>>> myRank=%d, buffsize %d  offsset %d\n", myRank, buff_size, offset);

        // recv the x
        MPI_Recv(x_p+offset, buff_size, MPI_DOUBLE, r, r * 10, MPI_COMM_WORLD, &status);
        
        // MPI_Recv(x_p+offset + buff_size, buff_size, MPI_DOUBLE, r, r * 100, MPI_COMM_WORLD, &status);
