...
#pragma omp parallel for default(shared) private(i1,i2) num_threads(numThreads) reduction(max:maxErr) reduction(max:maxNorm)
for( i2=n2a; i2<=n2b; i2++ )
for( i1=n1a; i1<=n1b; i1++ )
{
...
printf("option=%s: numThreads=%3d, numSteps=%d nx=%d maxNorm=%8.2e maxRelErr=%8.2e cpuTimeStep=%9.2e(s)\n",
...
fprintf(matlabFile,"dx(1)=%14.6e; dx(2)=%14.6e; numGhost=%d; numThreads=%d;\n",dx[0],dx[1],numGhost,numThreads);
