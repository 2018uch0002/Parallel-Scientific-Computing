...
extern "C"
{
  ...
  Real & un, const Real &u, const Real & rx, const Real &ry, const int& numThreads );
}
...
main(int argc, char *argv[])
{
  ...
  RealArray & u0 = ua[0];
  Real t=0.;
  #pragma omp parallel for shared(x, nd2a, nd2b, nd1a, nd1b, t) private(i2) num_threads(numThreads)
  for( i2=nd2a; i2<=nd2b; i2++ )
    for( i1=nd1a; i1<=nd1b; i1++ )
    {
  ...
  for( n=0; n<numSteps; n++ )
  {
    ...
    {
      #pragma omp parallel for shared(ua, x, rx, ry, n2a, n2b, n1a, n1b) private(i1, i2) num_threads(numThreads)
  ...
  #pragma omp parallel for default(shared) private(i2, i1) reduction(max: maxErr, maxNorm) num_threads(numThreads)
  for( i2=n2a; i2<=n2b; i2++ )
    for( i1=n1a; i1<=n1b; i1++ )
    {
  ...
  printf("option=%s: numThreads=%d numSteps=%d nx=%d maxNorm=%8.2e maxRelErr=%8.2e cpuTimeStep=%9.2e(s)\n",
  optionName.c_str(), numThreads, numSteps,nx,maxNorm,maxErr,cpuTimeStep);
  ...
  if( saveMatlab )
  {
    ...
    fprintf(matlabFile,"xa=%g; xb=%g; ya=%g; yb=%g; kappa=%g; t=%g; maxErr=%10.3e; cpuTimeStep=%10.3e, numThreads=%d;\n",
    xa,xb,ya,yb,kappa,tFinal,maxErr,cpuTimeStep, numThreads);
