...
int main( int argc, char* argv[] )
{
    ...
    int i;
    #pragma omp parallel for shared(u_p, x_p, t) private(i) num_threads(numThreads)
    for( i=nd1a; i<=nd1b; i++ ){
        uc(i)=UTRUE(x(i),t);
    }
    ...
    for( int n=0; n<numSteps; n++ )
    {
        ...
        int i;
        #pragma omp parallel for shared(u_p, x_p, rx, dt, t) private(i) num_threads(numThreads)
        for(i=n1a; i<=n1b; i++ )
        {
    ...
    #pragma omp parallel for shared(u_p, error_p, x_p, t) private(i) reduction(max:maxErr) num_threads(numThreads)
    for( i=nd1a; i<=nd1b; i++ )
    {
