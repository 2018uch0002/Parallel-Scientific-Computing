...
Real getMaxResidual( RealArray & u, RealArray & f, PoissonParameters & par )
{
  ...
  Real maxRes=0.;
  int i1 = 0, i2 = 0;
  #pragma omp parallel for default(shared) private(i1, i2) reduction(max:maxRes)
...
Real getMaxError( RealArray & u, RealArray & err, PoissonParameters & par )
{
  ...
  Real maxErr=0.;
  int i1 = 0, i2 = 0;
  #pragma omp parallel for default(shared) private(i1, i2) reduction(max: maxErr)
...
// Jacobi iteration
// ---------------------------------------------
int jacobiIteration( RealArray & u, RealArray & f, PoissonParameters & par, const int& numThreads )
{
  const IntegerArray & gridIndexRange = par.gridIndexRange;
  const IntegerArray & dimension = par.dimension;
  const IntegerArray & boundaryCondition = par.boundaryCondition;
  ...
  Real cpu1 = getCPU();
  int i1 = 0, i2 = 0, numIterations=0;
  #pragma omp parallel default(shared) private(n, i1, i2) num_threads(numThreads)
  {
...
// Gauss-Seidel or SOR
// ---------------------------------------------
int gaussSeidelIteration( RealArray & u, RealArray & f, PoissonParameters & par, const int& numThreads )
{
  const IntegerArray & gridIndexRange = par.gridIndexRange;
  const IntegerArray & dimension = par.dimension;
  const IntegerArray & boundaryCondition = par.boundaryCondition;
...
int redBlackIteration( RealArray & u, RealArray & f, PoissonParameters & par, const int& numThreads  )
{
...
// ---------------------------------------------
int conjugateGradientIteration( RealArray & u, RealArray & f, PoissonParameters & par, const int& numThreads )
{
  const IntegerArray & gridIndexRange = par.gridIndexRange;
  const IntegerArray & dimension = par.dimension;
  const IntegerArray & boundaryCondition = par.boundaryCondition;
  ...
  // rNormSquared = r^T r
  rNormSquared=0.;
  #pragma omp parallel for default(shared) private(i1, i2) reduction(+: rNormSquared) num_threads(numThreads)
  for( i2=n2a+1; i2<=n2b-1; i2++ )
    for( i1=n1a+1; i1<=n1b-1; i1++ )
    {
  ...
  for( n=0; n<maxIterations; n++ )
  {
    ...
    #pragma omp parallel for default(shared) private(i1, i2) reduction(+: pz) num_threads(numThreads)
    for( i2=n2a+1; i2<=n2b-1; i2++ )
      for( i1=n1a+1; i1<=n1b-1; i1++ )
      {
    ...
    rNormSquaredNew=0.;
    #pragma omp parallel for default(shared) private(i1, i2) reduction(+: rNormSquaredNew) num_threads(numThreads)
    for( i2=n2a+1; i2<=n2b-1; i2++ )
      for( i1=n1a+1; i1<=n1b-1; i1++ )
      {
...
main(int argc, char *argv[])
{
  ...
  printf("   nx=%d, ny=%d, maxIterations=%d, tol=%9.2e, numThreads=%d\n",nx,ny,maxIterations,tol, numThreads);
  printf("-----------------------------------------------------------------\n");
