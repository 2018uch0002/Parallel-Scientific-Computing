...
main(int argc, char *argv[])
{
  ...
  printf("   saveMatlab=%d, matlabFileName=%s, numThreads=%d \n",saveMatlab,matlabFileName.c_str(), numThreads);
  ...
  Real *rhsx_p = rhsx.getDataPointer() + thread_id * Ngx;
  Real *rhsy_p = rhsy.getDataPointer() + thread_id * Ngy;
  
  for(n=0; n<Nt; n++ ) // time-step loop
  {
    t = n*dt; // current time
    th = t+.5*dt; // half time
    tn = t+dt; // new time
    ...
    for( i2=n2a+1; i2<=n2b-1; i2++ ) // exclude top an bottom boundaries
    {
      ...
      // --- forward elimination ---
      for( int k=iax+1; k<=ibx; k++ )
      {
          rhsx_p[k] += Ax(0,k)*rhsx_p[k-1];
      }
      ...
      // [             bm cm ][xm]   [rm]
      // [                cn ][xn]   [rn]
      rhsx_p[ibx] = rhsx_p[ibx]/Ax(1,ibx);
      for( int k=ibx-1; k>=iax; k-- )
      {
          rhsx_p[k] = (rhsx_p[k] - Ax(2,k)*rhsx_p[k+1] )/Ax(1,k);
      }
      
      for( i1=n1a; i1<=n1b; i1++ )
        U(i1,i2) = rhsx_p[i1]; // save U^* in U
    }
    ...
    {      
      ...
      // --- forward elimination ---
      for( int k=iay+1; k<=iby; k++ )
      {
          rhsy_p[k] += Ay(0,k)*rhsy_p[k-1];
      }
      ...
      // [             bm cm ][xm]   [rm]
      // [                cn ][xn]   [rn]
      rhsy_p[iby] = rhsy_p[iby]/Ay(1,iby);
      for( int k=iby-1; k>=iay; k-- )
      {
          rhsy_p[k] = (rhsy_p[k] - Ay(2,k)*rhsy_p[k+1] )/Ay(1,k);
      }
  ...
  #pragma omp parallel default(shared) private(i1, i2) reduction(max: maxErr, maxNorm) num_threads(numThreads)
  for( i2=n2a; i2<=n2b; i2++ )
    for( i1=n1a; i1<=n1b; i1++ )
    {
  ...
  fprintf(matlabFile,"xa=%g; xb=%g; ya=%g; yb=%g; kappa=%g; t=%g; maxErr=%10.3e; cpuTimeStep=%10.3e; cpuTriSolves=%10.3e; numThreads=%d\n",
  xa,xb,ya,yb,kappa,tFinal,maxErr,cpuTimeStep,cpuTriSolves, numThreads);
