...
int main( int argc, char* argv[] )
{
    ...
    Real *w_p; // create for w_i to store the intermediate calculations
    w_p = new Real[nd1]; // allocate the space for the w_p

    // Macros to define fortran like arrays
    #define uc(i) u_p[cur ][i-nd1a]
    #define un(i) u_p[next][i-nd1a]
    #define w_p(i) w_p[i-nd1a]
    ...
    for( int n=0; n<numSteps; n++ )
    {
        ...
        // --- stage-1: update the interior points for "w" ----
        for( int i=n1a; i<=n1b; i++ )
        {
            w_p(i) = uc(i) + 0.5*rx*( uc(i+1) - 2.*uc(i) + uc(i-1) ) + (dt*0.5)*FORCE( x(i), t );
        }
        ...
        for( int side=0; side<=1; side++ )
        {
            ...
            {
                w_p(i) = UTRUE(x(i), t + dt*0.5);
                w_p(i-is) = 3.*w_p(i) - 3.*w_p(i+is) + w_p(i+2*is); // extrapolate ghost
            }
            else
            {
                // Neumann BC
                w_p(i-is) = w_p(i+is) - 2.*is*dx*UTRUEX(x(i),t+dt*0.5);
            }
        }

        // --- stage-2: update the interior points for "u" ----
        for( int i=n1a; i<=n1b; i++ )
        {
            un(i) = uc(i) + rx*( w_p(i+1) - 2.*w_p(i) + w_p(i-1) ) + dt*FORCE( x(i), t + dt*0.5 );
        }
